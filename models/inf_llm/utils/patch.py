...
def patch_hf(
    ...
    **kwargs
    ...
    # 1. 取得 RoPE 维度；新版本没有 .dim，需要自行计算
    rope_dim = getattr(hf_rope, "dim", hf_rope.inv_freq.shape[0] * 2)

    # 2. 取得 base，若不存在则退回默认值 10000
    base = base if base is not None else getattr(hf_rope, "base", 10000)
